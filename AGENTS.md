...
- **`export-configuration-file`: unsupported.** vRA 8 serves a single configuration element as JSON only and answers an artifact request with `406`, while the same request for a workflow or an action returns the artifact. Read it with `get-configuration`, or add it to the project package and export that.
...
Two write bodies differ by platform in that mode, both verified in the lab: configuration writes carry the plural `attributes` key (vRA 8 answers `400` for the singular `attribute`, even alongside the plural one), and `PUT /configurations/{id}` must carry the element name, so a call that does not rename reads the live name first.
