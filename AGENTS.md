...
- The current `scaffold-workflow-file` contract emits scriptable task workflow items. For a single-action wrapper that must use a native action item, start from an exported valid workflow/package shape or manually author the native action item before package publishing.
