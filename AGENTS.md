...
Two configuration write bodies differ by platform in that mode, both verified in the lab: configuration writes carry the plural `attributes` key (vRA 8 answers `400` for the singular `attribute`, even alongside the plural one), and `PUT /configurations/{id}` must carry the element name. On both platforms that `PUT` replaces the element, so `update-configuration` reads the live element once — before the `confirm` gate — and carries an omitted name and description forward; an omitted attribute set on an element that has attributes is refused instead, naming the secure ones whose values must be supplied fresh, and a secure attribute sent without a value is refused.
