...
- **`import-configuration-file`: unverified.** The endpoint is reachable and the multipart transport is proven by the workflow, resource, and package imports, but vRA 8 cannot export a `.vsoconf`, so no genuine container was round-tripped. It is not guarded.
