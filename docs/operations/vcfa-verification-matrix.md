...
| `create-template` / `delete-template` | Verified under VCFO-099 | Both exercised on a 9.x tenant session against a disposable DRAFT blueprint, with all four `delete-template` guard arms. **`create-template` requires non-empty `content` on 9.1** — its description said otherwise until VCFO-101; see the tenant section below. Blocked by identity on a provider session — `500` confirmed. |
...
| `create-template` | Verified, with a platform constraint | Created a disposable DRAFT blueprint. **`content` is not optional on 9.1**: omitting it answers `400 {"message":"Unknown Server Validation Error"}`. A minimal `formatVersion: 1` is accepted. The tool's description said an empty template is created when `content` is omitted, which is true on vRA 8 and false here. **Fixed under VCFO-101**: the description states the constraint, and a `400` on a create that omitted `content` now carries that guidance appended to the server's own message. Not refused pre-emptively — the vRA 8 path works, and no 9.0 lab exists to generalize from 9.1. Both arms re-verified live on the same 9.1 tenant session: the `content`-less create answered the same `400 {"message":"Unknown Server Validation Error","statusCode":400}` with the guidance appended below the server's own body, and the identical call carrying `content: "formatVersion: 1"` created the blueprint in `DRAFT`. The disposable blueprint was deleted afterwards, its `expectedName` guard refusing a mismatched name first. |
...
- **Two defects VCFO-099 found, both since fixed:**
  ...
  - [#224](https://github.com/mgovedarov/mcp-vcf-orchestrator/issues/224) (VCFO-101) — `create-template` described `content` as optional and said an empty template is created when it
    is omitted. On 9.1 that request answers `400`. The description was correct for vRA 8 only.
    **Fixed under VCFO-101** by correcting every description that shipped the claim and
    translating the opaque `400` into guidance when the create omitted `content`.
