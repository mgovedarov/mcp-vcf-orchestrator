...
| `export-configuration-file` | Unsupported — vRA 8 serves a configuration element as JSON only and answers the artifact request with `406`. Use `get-configuration`, or add it to the project package and export that. `prepare-artifact-promotion` reports a configuration backup as skipped for the same reason and still returns its report. The same `406` was later observed on a standalone vRO 9.1, so the refusal is not necessarily specific to vRA 8; a `406` from any platform now reports this guidance instead of the server's HTML body |
| `import-configuration-file` | Unverified but not guarded — the endpoint is reachable and the multipart transport is proven, but no genuine container has been round-tripped. vRA 8 cannot export a `.vsoconf`, and the vRO 9.1 lab refuses the export in the same way, so a container has yet to be sourced from any environment |
