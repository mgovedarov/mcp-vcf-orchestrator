...
If the request ID was not captured — the action was submitted in an earlier session, by another operator, or through the platform UI — recover it with `list-deployment-requests(deploymentId: "<deployment-id>")`, which lists everything ever requested against the deployment and marks each row with its action or as the create request. That listing is also the only way to reach the **create** request's ID: the catalog request answers a deployment ID and name and no request ID at all, so it is how a provision in flight is followed with task progress rather than by polling `get-deployment` for a terminal status.
