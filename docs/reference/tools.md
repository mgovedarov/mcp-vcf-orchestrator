...
The same `406` has since been observed on vRO 9.1, both standalone and appliance-embedded: `application/zip`, `application/vcoobject+zip` and `application/octet-stream` are all refused, while `*/*` returns the element as JSON. On those same hosts workflow, action and package exports answer `200 application/zip`, so the refusal is specific to configuration elements rather than to the host or the identity, and is not specific to vRA 8. No vRO tested serves a `.vsoconf` for a single element. Only the `vra8` refusal is pre-emptive; on any other platform the request is sent, and a `406` is reported with the same guidance rather than as the server's HTML error page.
...
This is the authoritative progress read for a day-2 action: a deployment's own status may remain `CREATE_SUCCESSFUL` throughout an action, observed on both platforms. The route is `GET /deployment/api/requests/{id}`, **verified live on vRA 8.18 and on VCF Automation 9.1** under VCFO-095, which drove this tool against five real requests on each platform; both serve the same response keys.

Observed status sequence: `PENDING` → `INITIALIZATION` → `CHECKING_APPROVAL` → `INPROGRESS` → `SUCCESSFUL`, with `COMPLETION` between the last two on 9.1. The complete service vocabulary is wider still — `FAILED` and an approval hold have never been produced — so an unfamiliar status is treated as potentially active rather than finished.
...
Delete a deployment by its ID. This is a destructive live operation, and an asynchronous one: the service answers with the `Deployment.Delete` request it queued, the deployment reads `DELETE_INPROGRESS` for a while (about 45 s on the vRA 8.18 lab) and then answers `404`. The tool therefore reports the deletion as *requested* — with the request ID and status when the service returns them — and names `get-deployment` (poll to `404`) and `list-deployments` (poll to absence) as the way to confirm it; a success line here is not evidence that the resources are gone. When the request's status is not a running one (`PENDING`, `INITIALIZATION`, `CHECKING_APPROVAL`, `INPROGRESS`, `COMPLETION`), the tool says so instead of promising a `404` — a request held by an approval policy never reaches `DELETE_INPROGRESS`. The delete itself is verified live on VCF Automation 9.1 (VCFO-074) and on vRA 8.18 in `vra8` mode (VCFO-088), and its request body and status vocabulary are now observed on both platforms (VCFO-095), where the request reached `SUCCESSFUL` on the same poll the deployment answered `404`.
...
On success the tool renders the `DeploymentRequest` the service queued and, when it includes an ID, directs the caller to poll `get-deployment-request` until it reports a confirmed terminal or held status. Observed active statuses are `PENDING`, `INITIALIZATION`, `CHECKING_APPROVAL`, `INPROGRESS`, and `COMPLETION`; because the complete service vocabulary is wider still, an unfamiliar status is treated as potentially active rather than finished by default. For `Deployment.Delete`, the tool additionally says to verify that `get-deployment` answers `404` or `list-deployments` no longer includes the deployment. For any other action, confirm the outcome on the deployment's resources after the request succeeds. A request whose status is known to be non-running is reported as held or finished rather than as something to wait for. Verified live on vRA 8.18 in `vra8` mode under VCFO-088 — the first day-2 submission on any platform — and on VCF Automation 9.1 under VCFO-095, which submitted the first day-2 action on 9.x and found the same request body and vocabulary, plus a `COMPLETION` status vRA 8.18 does not show.
