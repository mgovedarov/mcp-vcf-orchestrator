...
The same `406` has since been observed on one standalone vRO 9.1 appliance — `application/zip`, `application/vcoobject+zip` and `application/octet-stream` are all refused, while `*/*` returns the element as JSON — so the refusal is not necessarily specific to vRA 8. Whether it generalizes is untested: no successful artifact export was available on that host as a control. Only the `vra8` refusal is pre-emptive; on any other platform the request is sent, and a `406` is reported with the same guidance rather than as the server's HTML error page.
