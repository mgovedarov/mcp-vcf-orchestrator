...
- `VCFA_VRO_HOST` points the vRO `/vco/api` requests at an external vRO appliance while the login, the `GET /api/versions` probe, and the five Automation services stay on `VCFA_HOST`, on `vcfa`, `vcfa9.x`, and `vra8` alike. No second login runs: the token the Automation appliance issues is accepted verbatim by the external vRO, verified live on VCF Automation 9.1 with an external vRO 9.1 (the vRA 8 topology is wired identically but not yet live-verified). Unset, blank, or equal to `VCFA_HOST`, nothing changes. While the two hosts differ, every `[vro-client]` log line and every `vRO API error` ends with ` on <host>`, so a `403` with an HTML body — the Automation appliance's embedded orchestrator refusing a tenant whose organization uses an external vRO — is attributable at a glance; with the variable unset, that same `403` now carries a hint naming it. The server logs the routing once at startup (VCFO-081).

### Changed

- Authenticated API requests no longer follow redirects. Fetch strips `Authorization` on a cross-origin redirect, so a followed `302` could only ever reach a UI or SSO page unauthenticated, and it then surfaced as a `200` HTML body failing JSON parsing with no host named; the `3xx` is now reported with its status, the redirect target's host (never its query), and the variable to check. Transport failures name the host they were addressed to — `Request to vro.example.com failed: ENOTFOUND` rather than a bare `fetch failed` — with the original error kept as `cause`. Both `VCFA_HOST` and `VCFA_VRO_HOST` are trimmed, and a value carrying a URL scheme is rejected at startup with an actionable message, since `https://host` could never have connected; `VCFA_VRO_HOST` also rejects a path (VCFO-081).
