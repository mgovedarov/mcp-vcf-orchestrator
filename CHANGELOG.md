...
- `list-workflows` accepts an optional `limit` (1–1000) that caps how many items are returned, with early-stop pagination so the server isn't asked for more pages than needed. When `limit` drops items that exist on the server, the result carries a `limited` flag and the tool output ends with a notice (VCFO-064).
