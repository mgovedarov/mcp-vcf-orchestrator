...
- **Subscription writes in `vra8` mode sent a request shape vRA 8 rejects**, which the Automation-write guard had been hiding. Three differences, all verified live: vRA 8 requires the client to choose the subscription id (a body without one answers `500 The given id must not be null`), it answers the create `201` with an empty body and the created element in the `Location` header, and it serves neither `PUT` nor `PATCH` on `/subscriptions/{id}` (both `405`). `create-subscription` now generates a UUID and re-reads the created element — without the re-read the tool rendered the empty `{}` a bodyless response parses to, printing `Name: undefined`. `update-subscription` now reads the live element, merges the caller's changes over the caller-settable fields only (never server-owned ones such as `orgId`, `ownerId`, or `subscriberId`), and upserts the whole body with a `POST`. A partial upsert cannot silently discard fields the way the configuration `PUT` could: vRA 8 rejects one with `400 Property: eventTopicId must not be blank` before mutating anything. The `vcfa` path is unchanged (VCFO-070).
