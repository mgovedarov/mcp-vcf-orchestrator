...
- Added `test/action-export-reference.test.mjs`: the artifact export resolves a fully qualified action name to its element id, leaves a plain long-hex id untouched, and the live diff source resolves the same way (VCFO-076).
- Added `test/resource-category-guard.test.mjs`: `update-resource-element` and `delete-resource-element` refuse `expectedCategoryName` with an explanation when the element carries no category, still compare it when one is reported, and proceed when it is omitted (VCFO-077).
