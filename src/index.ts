...
async function main(): Promise<void> {
  ...
  const server = new McpServer(
    ...
    {
      instructions: [
        "This server connects to a VCF Automation instance by default, or to vRA/vRO 8.12+ (vIDM bearer-token auth) when VCFA_TARGET_PLATFORM is set to vra8. In vra8 mode the whole vRO surface is available, the Automation services can be read, and create-template, delete-template, and the subscription create/update/delete tools are supported. Deployment writes (create-deployment, delete-deployment, run-deployment-action) and export-configuration-file are not supported; route a configuration element through the project package instead.",
