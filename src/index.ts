...
async function main(): Promise<void> {
  ...
  const server = new McpServer(
    ...
    {
      instructions: [
        "This server connects to a VCF Automation instance by default, or to vRA/vRO 8.12+ (vIDM bearer-token auth) when VCFA_TARGET_PLATFORM is set to vra8. In vra8 mode the whole vRO surface is available, the Automation services can be read, and their writes are supported: create-template, delete-template, the subscription create/update/delete tools, and create-deployment, delete-deployment, and run-deployment-action. export-configuration-file is refused pre-emptively there, but that limit is not specific to vra8: no vRO tested, on either platform, serves a single configuration element as a .vsoconf artifact, so the request answers 406 elsewhere and is reported with the same guidance. Read the element with get-configuration, or route it through the project package instead.",
        ...
        "Use prepare-artifact-promotion before artifact imports when you need preflight, optional live backup export, workflow/action diff summaries, and the exact confirmed import call; it never imports. Direct imports should include the expected target guard when available: expectedCategoryId or expectedCategoryName for workflow/configuration/resource imports, expectedCategoryName (the target module) for action imports, or expectedPackageName for packages.",
        ...
        "Use list-deployments to see existing deployments; use create-deployment with confirm set to true to deploy a catalog item, providing the catalogItemId, deploymentName, and projectId. Use list-deployment-actions to discover available deployment day-2 actions, then run-deployment-action with confirm set to true and expectedDeploymentName and expectedActionName to submit one. Use list-deployment-requests to list what has been requested against a deployment and to obtain a request ID -- including the create request's, which is returned nowhere at submission -- then get-deployment-request to follow it to a terminal status.",
