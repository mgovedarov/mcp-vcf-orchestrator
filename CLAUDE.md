...
Toolchain: Node `>=22.19.0` (`engines`, driven by `undici` 8); `.nvmrc` pins 24. CI runs `npm test` on Node 22 and 24 and the full `npm run validate` gate on 24. Docs deploy to GitHub Pages from `main`; npm publishing runs only for published GitHub releases.
...
- `VCFA_TARGET_PLATFORM` – `vcfa` (default, VCF Cloud API session flow with API version auto-negotiation via `GET /api/versions`, preferring `9.1.0` then `9.0.0`), `vcfa9.1`/`vcfa9.0` (pin the VCF Cloud API version, skipping the probe), or `vra8` (vRA/vRO 8.12+, authenticated with the vRA 8 bearer-token flow: vIDM CSP login with `VCFA_ORGANIZATION` as the domain, refresh-token exchange at `/iaas/api/login`, `Authorization: Bearer` on `/vco/api`). `vra8` supports the full vRO `/vco/api` surface, reads and writes alike, plus Automation-service **reads** (catalog items, deployments, templates, projects, subscriptions, event topics), all verified against a vRA 8.18 lab under VCFO-068. Blueprint-service and event-broker **writes** (`create-template`, `delete-template`, and the subscription create/update/delete tools) are verified against that lab under VCFO-070 and supported. Catalog-service and deployment-service writes (`create-deployment`, `delete-deployment`, `run-deployment-action`) remain unsupported pending verification, as does `export-configuration-file`, since vRA 8 serves a configuration element as JSON only and answers the artifact request with `406` — route it through the project package instead. `VCFA_ORGANIZATION=system` (case-insensitive) routes the login to the provider sessions endpoint for provider/system administrators; tenant users set their org name (URL slug, not display name). In `vra8` mode `VCFA_ORGANIZATION` is the vIDM domain shown on the Workspace ONE login page, for example `System Domain`.
...
1. **Client layer (`src/client/`)** – Modular VCF/vRO clients split by responsibility. `core.ts` owns auth, HTTP, and bearer-token refresh; `index.ts` composes the per-domain clients (workflow, action, configuration, category, subscription, catalog, deployment, template, package, resource, plugin, project) into the facade. Cross-cutting helpers live alongside them: `pagination.ts`, `parameters.ts`, `attrs.ts`, `files.ts` (artifact path safety), `artifact-preflight.ts`, `workflow-artifact.ts`, and `context-snapshot.ts`. `src/vro-client.ts` is a stable compatibility shim re-exporting `VroClient` for the public import path. All HTTP goes through the npm `undici` package's own `fetch`, `FormData`, and `Agent` (imported in `core.ts`), never Node's global `fetch`: the runtime-bundled undici is a different major and silently mis-serializes a foreign `FormData` (breaking multipart imports) and ignores the `VCFA_IGNORE_TLS` dispatcher. Unit tests may still stub `globalThis.fetch`; `core.ts` detects a swapped global and honors it.
2. **Tool layer (`src/tools/`)** – Each file registers a group of MCP tools and maps natural-language tool names onto client calls. Shared infrastructure: `confirmation-guards.ts` enforces `confirm: true` and the `expected*` target guards on mutating tools; `annotations.ts` exports `DESTRUCTIVE_LIVE_WRITE`, which every `import-`/`delete-`/`update-`/`run-` tool must use (and be listed in `test/tool-annotations.test.mjs`, which fails otherwise); `truncation.ts` renders the page-cap warning that `list-*` results surface instead of silently returning a partial inventory; `content-summary.ts` summarizes bulky raw content (scripts, XML, YAML) that `get-*` tools omit by default behind an opt-in flag. `promotion-tools.ts` and `context-tools.ts` implement the artifact-promotion and context-snapshot surfaces.
...
- **Discovery-first, conservative writes**: Read-only `list-*`/`get-*` tools come first. Mutating tools (create/update/import/delete, deployment day-2 actions, package import, local overwrite) require an explicit `confirm: true` argument enforced via `confirmation-guards.ts` — and that schema flag is in addition to, not a substitute for, real user confirmation of target and impact. Mutations also accept expected-target guards (`expectedName`, `expectedCategoryId`/`expectedCategoryName`, `expectedPackageName`, `expectedWorkflowName`/`expectedInputNames`, `expectedDeploymentName`/`expectedActionName`) that are verified against live metadata before the write; new mutating tools should offer the same two-phase flow.
- **Never print secrets**: do not echo configuration attribute values, workflow/action scripts that embed credentials, tokens, passwords, or private keys in tool output, logs, tests, or docs. Prefer names, IDs, types, descriptions, and redacted summaries.
...
- **Issue codes**: use `VCFO-###` (sequential, prefixed in issue titles, e.g. `[VCFO-001] ...`). Planning lives in GitHub Issues; `TODO.md` is deprecated and kept for history only.
- **Changelog**: `CHANGELOG.md` keeps an `## Unreleased` section at the top with `### Added` / `### Changed` / `### Fixed` / `### Dependencies` / `### Docs` / `### Tests` subsections as needed. Add an entry for every user-visible change and reference the issue code (e.g. `(VCFO-063)`). Releases are cut by renaming that section to `## <version> - <date>`; the runtime server version is read from `package.json`, so never hardcode it (see `test/version-sync.test.mjs`).

## Testing & Validation

Automated tests use Node's built-in test runner over `test/*.test.mjs`. Tests import the compiled output from `dist/` (not `src/`), so `npm test` builds first and any direct `node --test` invocation must be preceded by `npm run build` or it will run against stale output. Tests never contact a live VCFA environment: clients are exercised with in-memory fakes, a stubbed `globalThis.fetch`, or a loopback TLS server, and `test/index-startup.test.mjs` spawns the built `dist/index.js` with a controlled environment to cover startup failures and graceful `SIGTERM` shutdown. Use the smallest meaningful check for a change and the full `npm run validate` gate before broad, docs, or release-facing changes.
