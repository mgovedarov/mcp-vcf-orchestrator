...
16. [x] Add `diff-workflow-file` to compare two local `.workflow` artifacts, or a live workflow export against a local artifact, and summarize meaningful changes to parameters, attributes, scripts, bindings, task flow, and action references
17. [x] Add `diff-action-file` to compare `.action` artifacts, or a live exported action against a local artifact, and highlight script, parameter, return type, module, and name changes
18. [x] Add `prepare-artifact-promotion` to run preflight for a workflow, action, configuration, or package artifact; optionally export the current live target as a backup; summarize risks and changes; then recommend the exact import tool call
19. [x] Moved to GitHub issue [VCFO-001]: Expand MCP resources and prompts for workflow/template implementation — https://github.com/mgovedarov/mcp-vcf-orchestrator/issues/18
20. [x] Moved to GitHub issue [VCFO-002]: Collect and persist reusable VCF Automation/vRO context — https://github.com/mgovedarov/mcp-vcf-orchestrator/issues/19
